...
class Signal():
    def __init__(self, data,kind, ratio, indicator,BB_std , rolling_window, currencypair, lotsize):
        self.data = data
        self.kind = kind
        self.Ratio = ratio 
        ...
        for i in range(self.TicketCounter-1,Minumum-1,  -1):                        
            if  (TradeTicketDict["BuySell"].iloc[i] == "Buy"):    
                if (PriceDiff < -StopLossPips*0.0001) and (TradeTicketDict["OpenClose"].iloc[i] == 1): # open = 1, close = 0
                    TradeTicketDict["P&L(USD)"].iloc[i] = (PriceDiff/indicator_data["Close"].iloc[row])*(100000*self.lotsize)
                    TradeTicketDict["OpenClose"].iloc[i] = 0 
                    TradeTicketDict["ClosePosPrice"].iloc[i] = indicator_data["Close"].iloc[row]
                    TradeTicketDict["CloseDate"].iloc[i] = indicator_data.index[row].strftime("%Y-%m-%d")
                    self.BuyPosition = 0                    
    ...
    def Strategy(self):
        TradeTicketDict = pd.DataFrame(columns = ["Currency", "OpenDate","CloseDate","TradeTicket","BuySell","OpenClose","Price","ClosePosPrice",  "P&L(USD)"],index = np.arange(200))
        counter = 0
        ...
        for row in range(3,self.indicator_data.shape[0]):
            ...
            if (CurOpenClose > np.mean(PreOpenCloseOne+PreOpenCloseTwo+PreOpenCloseThree)):
                Momentum += 1 
            ...
            if  (BuyReversal+Momentum > 1) and (self.BuyPosition == 0) : 
                ...
                TradeTicketDict["Currency"].iloc[self.TicketCounter] = self.CurrencyPair
                TradeTicketDict["OpenDate"].iloc[self.TicketCounter] = self.indicator_data.index[row].strftime("%Y-%m-%d")
